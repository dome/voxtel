...
<script>
	...
	demo.channels = function(msgs) {
		...
		$('statusbar').innerHTML = "Ready"+$('myexten').value;
	}
	...
	demo.pongs  = function(msgs) {
		resp = msgs[0].headers['response'];
		if (resp == "Pong") {
			$('statusbar').innerHTML = "<i>Already connected...</i>"+$('username').value;
			loggedOn();
		} else {
	...
	function localajaminit() {
		...
		//var userwin = window.open("/userdetail/","userdetail");
		doLogin();
	}
	//doLogin();
</script>

<title>TuxZA Call center</title>
<body onload="localajaminit()">
<table align="center" width=600>
<tr valign="top"><td>
<table align="left">
<form name="tuxform">
<tr><td><input type="hidden" id="username" value="<?echo $_REQUEST['username']?>"></td></tr>
<tr><td><input type="hidden" id="secret" value="<?echo $_REQUEST['secret']?>"></td></tr>
<tr><td><input type="hidden" id="myexten" value="<?echo $_REQUEST['exten']?>"></td></tr>
</form>

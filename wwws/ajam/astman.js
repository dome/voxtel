...
function TuxAstMan() {
	...
	this.channelTable = function(callback) {
		...
		count=0;
		jsvar = document.tuxform.myexten.value;
		for (x in channels) {
			 if (channels[x].channel) {
				//self.focus();
				//alert(channels[x].callerid);
				if (count % 2)
					cclass = "chanlistodd";
				else
					cclass = "chanlisteven";
				...
				var s1 = channels[x].channel.split("-",1);
				var s2 = s1[0].split("/");
				var myexten = s2[1];
				//document.write(s2[1]+ "<br />");
				if (jsvar.length > 0 && myexten==jsvar) {
				    if(typeof(winRef) == 'undefined' || winRef.closed){
				    //create new
					winRef = window.open("/b/userdetail/"+channels[x].callerid);
				    } else {
    				    //it exists, load new content (if necs.)
    					winRef.location.href = "/b/userdetail/"+channels[x].callerid;
